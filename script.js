...
let gridTetris = Array.from({
	length: qtdTilesY
}, () => Array(qtdTilesX).fill(null));
let peçaTetris = [];
let naves = [];
let balaAtual = null;
let balasJogador = [];
const velocidadeJogo = 1000;
let velocidadeTetris = velocidadeJogo / 4;
...
function colocarComida() {
	do {
		comida = {
			x: Math.floor(Math.random() * qtdTilesX),
			y: Math.floor(Math.random() * qtdTilesY)
		};
...
function prepararPeçaTetris() {
	...
	peçaTetris = peçaTetris.map(segmento => ({
		x: segmento.x,
		y: segmento.y - minY
	}));
	cobra = [];
